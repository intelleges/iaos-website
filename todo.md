...
- [ ] Fix Zod schema mismatch in backend TRPC router (documentType enum missing 'case_study')
- [ ] Test download flow end-to-end with all document types
- [ ] Verify all 7 Gate #2 completion criteria pass
- [ ] Save checkpoint after successful fix

## Pricing Calculator System (Complete Implementation)

### Database Schema
- [x] Add pricingQuotes table to schema with all fields including customerEmail
- [x] Run database migration

### Backend API (tRPC Router)
- [x] Create pricing calculator library with tier definitions and pricing logic
- [x] Create pricing router with calculate, saveQuote, listQuotes, updateQuoteStatus endpoints
- [x] Create Stripe invoice generation integration
- [x] Create PDF proposal generation with wkhtmltopdf
- [x] Register pricing router in main router

### Frontend - Pricing Calculator Page
- [x] Create /admin/pricing page with configuration form
- [x] Add customer profile section (name, email, industry, region)
- [x] Add tier selection UI (Basic, Professional, Advanced, Enterprise)
- [x] Add configuration inputs (users, suppliers, protocols, sites, partner types)
- [x] Add integration toggles (ERP, eSRS, Premium Support)
- [x] Add real-time pricing calculation display
- [x] Add Save Quote, Generate Invoice, Export PDF buttons
- [x] Add link to quote history

### Frontend - Quote History Dashboard
- [x] Create /admin/quotes page with quote list table
- [x] Add search functionality (customer name/email)
- [x] Add status filter (draft/sent/accepted/rejected)
- [x] Add tier filter
- [x] Add pagination (25 per page)
- [x] Add status update dropdown
- [x] Add link back to calculator

### Testing & Integration
- [x] Test complete quote creation workflow
- [x] Test Stripe invoice generation
